...
<body class="min-h-screen flex flex-col items-center justify-start p-3 sm:p-5">
    ...
    <header class="no-print w-full max-w-7xl bg-white border border-slate-200 rounded-2xl px-4 py-3 mb-4 shadow-sm flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2 bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-xl text-xs font-semibold text-slate-700">
            ...
            <span id="conn-status">ESP32 UART: Kutish holatida</span>
        </div>
    ...
    <main class="no-print w-full max-w-7xl grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
                    ...
                    <div class="flex items-stretch justify-between relative" style="height: 520px;">
                        ...
                        <div class="w-16 flex flex-col items-center justify-between py-2 z-10">
                            <div class="text-center">
                                ...
                                <div id="force-val-text" class="mono font-black text-rose-600 text-base mt-0.5">0.0 kg</div>
                            </div>
                            
                            <!-- LED Bar Graph Enclosure (30 Segments) with Bulletproof Inline Style -->
                            <div id="force-bar-container" style="width: 36px; height: 410px; background-color: #1a0808; border: 2px solid #4a1d1d; border-radius: 6px; padding: 4px; display: flex; flex-direction: column-reverse; justify-content: space-between; box-shadow: inset 0 2px 6px rgba(0,0,0,0.8); position: relative;">
                                <!-- LED Segments generated by JS -->
                            </div>

                            <div class="text-[11px] font-black text-slate-700 uppercase tracking-tighter">MASSAJ</div>
                        </div>
        ...
        <section class="lg:col-span-7 flex flex-col gap-4">
            ...
            <!-- 3. PERSISTENT EXAM RESULTS HISTORY LOG (IMTIHON NATIJALARI JURNALI) -->
            <div id="exam-history-card" class="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm flex flex-col gap-3">
                <div class="flex items-center justify-between border-b border-slate-100 pb-2.5">
                    <div class="flex items-center gap-2">
                        <span class="text-xs font-black text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
                            <i class="fa-solid fa-clock-rotate-left text-blue-600"></i> IMTIHON NATIJALARI TARI популяRI / JURNALI
                        </span>
                        <span id="history-count-badge" class="px-2 py-0.5 rounded-full text-[11px] font-bold bg-blue-50 text-blue-700 border border-blue-200">
                            0 ta
                        </span>
                    </div>

                    <button type="button" onclick="clearAllExamHistory()" class="px-2.5 py-1 bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold rounded-lg border border-rose-200 transition text-[11px] flex items-center gap-1 cursor-pointer">
                        <i class="fa-solid fa-trash-can text-xs"></i> Tarixni Tozalash
                    </button>
                </div>

                <!-- History Table / Records Container -->
                <div id="history-list-container" class="max-h-60 overflow-y-auto space-y-2 pr-1">
    ...
    <script>
        ...
        function renderExamHistory() {
            ...
            container.innerHTML = list.map(item => `
                <div class="flex items-center justify-between p-2.5 rounded-xl border ${item.passed ? 'bg-emerald-50/40 border-emerald-200' : 'bg-rose-50/40 border-rose-200'} text-xs">
                    <div class="flex flex-col">
                        ...
                        <div class="text-[11px] text-slate-500 mt-0.5">
                            📅 ${item.date} | Zarbalar: <b>${item.correctComps}/${item.totalComps}</b> | Nafas: <b>${item.correctVents}</b>
                        </div>
                    </div>

                    <div class="flex items-center gap-2">
                        <span class="px-2 py-0.5 rounded-lg text-xs font-black ${item.passed ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'}">
                            ${item.score}% (${item.passed ? "O'TDI" : "YIQILDI"})
                        </span>
                        
                        <button type="button" onclick="openHistoryRecord('${item.id}')" title="Protokolni ko'rish va chop etish" class="px-2.5 py-1 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-bold text-[11px] transition shadow-sm cursor-pointer">
                            <i class="fa-solid fa-print mr-1"></i> Protokol
                        </button>
                        
                        <button type="button" onclick="deleteSingleExamRecord('${item.id}')" title="O'chirish" class="p-1 text-slate-400 hover:text-rose-600 transition cursor-pointer">
