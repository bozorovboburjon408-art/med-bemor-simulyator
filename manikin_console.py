...
<body class="min-h-screen flex flex-col items-center justify-start p-3 sm:p-5">
        ...
        <table class="w-full border-collapse border border-black text-xs mb-4">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="border border-black p-2 text-center">38 - 55 kg (5 - 6 sm)</td>
    ...
    <script>
        ...
        function calculateComprehensiveScore(stats, avgBpm) {
            const totalComps = stats.totalComps || 0;
            if (totalComps === 0) return 0;

            const correctComps = stats.correctComps || 0;
            const depthPct = Math.min(100, Math.round((correctComps / totalComps) * 100));

            const recoilErrors = stats.recoilErrors || 0;
            const recoilPct = Math.max(0, 100 - Math.round((recoilErrors / totalComps) * 100));

            const posErrors = stats.posErrors || 0;
            const posPct = Math.max(0, 100 - Math.round((posErrors / totalComps) * 100));

            let bpmScore = 100;
            if (avgBpm >= 100 && avgBpm <= 120) bpmScore = 100;
            else if ((avgBpm >= 95 && avgBpm < 100) || (avgBpm > 120 && avgBpm <= 125)) bpmScore = 90;
            else if ((avgBpm >= 90 && avgBpm < 95) || (avgBpm > 125 && avgBpm <= 130)) bpmScore = 80;
            else if ((avgBpm >= 80 && avgBpm < 90) || (avgBpm > 130 && avgBpm <= 140)) bpmScore = 65;
            else if ((avgBpm >= 70 && avgBpm < 80) || (avgBpm > 140 && avgBpm <= 150)) bpmScore = 45;
            else bpmScore = 25;

            const totalVents = stats.totalVents || 0;
            const correctVents = stats.correctVents || 0;
            const stomachErrors = stats.stomachErrors || 0;
            const stomachPenalty = stomachErrors * 8;

            let composite = 0;
            if (totalVents > 0) {
                const ventPct = Math.min(100, Math.round((correctVents / totalVents) * 100));
                composite = (0.40 * depthPct) + (0.15 * recoilPct) + (0.15 * bpmScore) + (0.15 * posPct) + (0.15 * ventPct) - stomachPenalty;
            } else {
                composite = (0.50 * depthPct) + (0.20 * recoilPct) + (0.15 * bpmScore) + (0.15 * posPct) - stomachPenalty;
            }
        ...
        function finishExamManually(isRemote = false) {
            ...
            const total = liveStats.totalComps || 0;
            const correct = liveStats.correctComps || 0;
            ...
            const overallScore = calculateComprehensiveScore(liveStats, avgBpm);
        ...
        function renderProtocolFromRecord(record) {
            ...
            document.getElementById("print-res-correct-comps").innerText = record.correctPct >= 80 ? "A'lo darajada" : (record.correctPct >= 60 ? "Qoniqarli" : "Past ko'rsatkich");
            ...
            document.getElementById("print-res-vents").innerText = record.totalVents > 0 ? (record.correctVents === record.totalVents ? "Me'yorda" : "Qisman to'g'ri") : "Bajarilmadi";
